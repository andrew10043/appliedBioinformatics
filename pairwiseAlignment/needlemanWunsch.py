...
# 2. F(i - 1, j) - d
# 3. F(i, j - 1) - d
# Gap extend penalty (e) used in place of d if more than one adjacent gap

# Concurrently fill the T matrix based on which score defines the current cell
# of the F matrix

# Note that the default in the case of ties is to take the left score, then
# diag, then up (this is the default in the source code) -- changes to this
# order will change the final sequence match
...
# Define alignment utilizing the T matrix traceback
...
print("Pairwise sequence match:")

print("".join(alignA))
print("".join(alignB))

print("Final alignment score is " +
      str(F.iloc[F.shape[0] - 1, F.shape[1] - 1]) + ".")
